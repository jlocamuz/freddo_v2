...
def aplicar_nocturnidad_50_100(df: pd.DataFrame) -> pd.DataFrame:
    """Calcula Horas Extra 50%/100% por nocturnidad usando el adicional ORIGINAL."""
...
def aplicar_nocturnidad_50_100_real(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula Horas Extra 50%/100% por nocturnidad usando el adicional REAL
    (ya corregido por CJN). Genera columnas con sufijo ' Real'.
    """
    df["ADICIONAL_NOCTURNIDAD_REAL"] = pd.to_numeric(df["ADICIONAL_NOCTURNIDAD_REAL"], errors="coerce").fillna(0.0)

    col50r  = "Horas Extra 50% (por nocturnidad) Real"
    col100r = "Horas Extra 100% (por nocturnidad) Real"
    df[col50r]  = 0.0
    df[col100r] = 0.0
    ...
    has_add = df["ADICIONAL_NOCTURNIDAD_REAL"] > 0

    df.loc[has_add & is_100,  col100r] = df.loc[has_add & is_100,  "ADICIONAL_NOCTURNIDAD_REAL"]
    df.loc[has_add & ~is_100, col50r]  = df.loc[has_add & ~is_100, "ADICIONAL_NOCTURNIDAD_REAL"]
